...
def _acquire_at(lock_file: Path, name: str) -> bool:
    ...
    Сначала пробуем создать файл эксклюзивно (`open(..., "x")`) — это
    атомарно на уровне ОС, а не «прочитать пустой файл, потом записать»,
    как было раньше. Найдено 18.08.2026: два процесса, стартовавшие в
    одну и ту же секунду, оба читали пустой файл ДО того, как второй
    успевал его записать — оба решали, что первые, и Telegram потом
    падал на Conflict от двух getUpdates разом.
    """
    ensure_data_dir()
    payload = json.dumps({"pid": os.getpid(), "name": name})
    try:
        with open(lock_file, "x", encoding="utf-8") as f:
            f.write(payload)
        return True
    except FileExistsError:
        pass  # файл уже есть — смотрим, кто держит, ниже
