...
> **Сжатия контекста нет и не настраивается.** Раньше здесь значился
> `CONTEXT_COMPRESSION_THRESHOLD` — настройка ничего не делала и только
> показывала в `/status` цифру несуществующей функции. Вместо неё сделана
> нить разговора (см. ниже): сжимать длинную историю не нужно, если она
> изначально короткая.
...
Долгая память (`recall`) ищет по смыслу и достаёт то, что похоже на вопрос.
Для нити разговора этого мало: на «переделай, но короче» похожих фактов не
находится. Поэтому последние **12 реплик** (шесть обменов) хранятся отдельно
на пару «канал + пользователь» и подмешиваются в промпт последними — ближе
всего к самому вопросу.

- Длинные реплики режутся до 600 символов: важен ход разговора, а не каждое слово.
- Пауза дольше **6 часов** — разговор считается новым, вчерашнее не подмешивается.
- Файл `dialog_history.json` в папке данных: перезапуск бота нить не рвёт.
- Каналы изолированы — сказанное в Telegram не всплывёт в веб-чате.
- `/reset` стирает нить. Память фактов и заметки при этом не трогаются:
  «сменить тему» и «забудь, что я говорил» — разные просьбы.

Ничего из этого не настраивается через `.env` — размеры окна подобраны в коде
(`backend/services/dialog_history.py`).
