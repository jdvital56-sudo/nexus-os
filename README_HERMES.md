...
> `CONTEXT_COMPRESSION_THRESHOLD`, но сжимать нечего: диалог не хранит историю
> переписки — каждое сообщение уходит в модель отдельно, а контекст собирается
> заново из памяти (`recall`) и базы знаний Obsidian. Настройка ничего не
> делала и только показывала в `/status` цифру несуществующей функции.
